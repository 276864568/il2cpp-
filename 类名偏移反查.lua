...
local function main()
    ...
    if libstart==0 then print("没有找到libil2cpp.so。如果游戏是分裂的，反分裂它。") end
    metadata = get_metadata()
    if #metadata==0 then return print("找不到数据") end
    ...
    local menu = gg.prompt({"以十进制为单位输入最大偏移距", "转储字段", "转储方法", "深入搜索(缓慢)"}, {"1000"}, {"number", "checkbox", "checkbox", "checkbox"})
    ...
    for i, v in ipairs(originalResults) do --loop to check every addresses in search list
        ...
        if found==false then print("没有得到类名。可能被抵消太短了。") end
        print("\n")
    end
end
